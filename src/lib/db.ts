...
export async function updateTaskPriority(
	...
	const database = await getDb();
	await database.execute("UPDATE tasks SET priority = $1 WHERE id = $2", [
		priority,
		id,
	]);
}
...
export async function updateTaskTags(id: number, tags: string): Promise<void> {
	const database = await getDb();
	await database.execute("UPDATE tasks SET tags = $1 WHERE id = $2", [
		tags,
		id,
	]);
}
...
export async function updateTaskRecurrence(
	...
	const database = await getDb();
	await database.execute("UPDATE tasks SET recurrence = $1 WHERE id = $2", [
		recurrence,
		id,
	]);
}
...
export async function moveTask(
	...
	await database.execute("UPDATE tasks SET sort_order = $1 WHERE id = $2", [
		neighbour[0].sort_order,
		id,
	]);
	await database.execute("UPDATE tasks SET sort_order = $1 WHERE id = $2", [
		curOrder,
...
export async function getStreak(): Promise<number> {
	...
	if (
		rows[0].completed_date !== todayStr &&
		rows[0].completed_date !== yesterdayStr
	) {
		return 0;
	}
...
export async function updateTaskNotes(
	...
	const database = await getDb();
	await database.execute("UPDATE tasks SET notes = $1 WHERE id = $2", [
		notes,
		id,
	]);
}
...
export async function completeTaskWithChildren(
	id: number,
): Promise<{ parentAutoCompleted: boolean }> {
	const database = await getDb();
	...
	try {
		...
		if (task.length === 0) {
			...
			return { parentAutoCompleted: false };
		}
...
export async function createPomodoroSession(
	...
	if (id == null || id === 0) {
		throw new Error(
			"createPomodoroSession failed: lastInsertId is missing or 0",
		);
	}
	return id;
}
...
export async function listTasksByTimeBlock(
	timeBlockId: number,
	dateStr: string,
): Promise<Task[]> {
	const database = await getDb();
...
export async function updateTaskDate(
	...
	const database = await getDb();
	await database.execute("UPDATE tasks SET task_date = $1 WHERE id = $2", [
		taskDate,
		id,
	]);
}
...
export async function updateTaskTimeBlock(
	...
	const database = await getDb();
	await database.execute("UPDATE tasks SET time_block_id = $1 WHERE id = $2", [
		timeBlockId,
		id,
	]);
}
